...
const FEATURES = [
  ...
  },
  {
    ...
    title: "Commit Review",
    desc: "Push to main and get instant AI review comments on every changed line — automatically posted on your commit.",
    badge: "Push",
    badgeColor: { background: "#fef9c3", color: "#a16207" },
    files: [{ name: "push → main", label: "EVENT" }, { name: "commitReview.ts", label: "TS" }],
  },
  {
    icon: <Network size={18} color="#be185d" />,
    title: "Dependency Graph",
    desc: "Interactive D3.js graph showing how every file imports every other. Click nodes to inspect connections.",
    badge: "D3.js",
    badgeColor: { background: "#fce7f3", color: "#be185d" },
    files: [{ name: "DependencyGraph.tsx", label: "TSX" }, { name: "FileTree.tsx", label: "TSX" }],
  },
  {
    icon: <MessageCircle size={18} color="#065f46" />,
    title: "Chat with Repo",
    desc: "Ask 'where is auth handled?' and get an instant, accurate answer with exact file references.",
    badge: "Chat",
    badgeColor: { background: "#d1fae5", color: "#065f46" },
    files: [{ name: "/api/chat", label: "POST" }, { name: "fileSelector.ts", label: "TS" }],
  },
  {
    icon: <Lock size={18} color="#b91c1c" />,
    title: "GitHub OAuth + App",
    desc: "Sign in with GitHub OAuth. Install the GitHub App on any repo to enable auto PR and commit reviews.",
    badge: "Auth",
    badgeColor: { background: "#fee2e2", color: "#b91c1c" },
    files: [{ name: "next-auth", label: "PKG" }, { name: "GitHub App", label: "APP" }],
  },
];
...
function Card({ feat }: { feat: typeof FEATURES[0] }) {
  return (
    <div className="rounded-2xl p-4 shadow-2xl w-full" style={{ background: "rgba(241, 240, 255, 0.97)" }}>
      ...
      <div className="rounded-lg p-2 space-y-1.5" style={{ background: "#f8f7ff", border: "1px solid #ebe8ff" }}>
        {feat.files.map((f) => (
          <div key={f.name} className="flex items-center gap-2 font-mono text-[10px]">
            <span className="text-[9px] font-bold px-1.5 py-0.5 rounded shrink-0" style={feat.badgeColor}>{f.label}</span>
  ...
  return (
    <div className="w-full max-w-[1200px] mx-auto">
      ...
      <div className="text-center mb-12 px-4">
        ...
        <h2 className="font-extrabold text-white tracking-[-0.02em] leading-[1.1] mb-3" style={{ fontSize: "clamp(24px, 3vw, 40px)" }}>
      ...
      {!isDesktop && (
        <div className="flex flex-col items-center w-full px-4">
          {FEATURES.map((feat, i) => (
            <div key={feat.title} className="flex flex-col items-center w-full">
              {i !== 0 && (
                <div className="flex flex-col items-center py-1">
                  <div className="w-px h-5 bg-gradient-to-b from-white/20 to-white/10" />
                  <div className="w-1.5 h-1.5 rounded-full bg-white/25" />
                  <div className="w-px h-5 bg-gradient-to-b from-white/10 to-white/20" />
                </div>
              )}
              <Card feat={feat} />
            </div>
      ...
      {/* desktop scattered */}
      {isDesktop && (
        <div className="relative h-[1100px] overflow-hidden">
          {FEATURES.map((feat, i) => (
            <div
              key={feat.title}
              className="absolute w-[240px] transition-transform duration-300 hover:-translate-y-1"
              style={{
                top: POSITIONS[i].top,
